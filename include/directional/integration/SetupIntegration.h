...
inline void setup_integration(const directional::CartesianField &field,
  ...
  cut_mesh_with_singularities(meshWhole, field.singLocalCycles,
                              intData.face2cut);
  ...
  // hedra::dcel(D, meshWhole.F, meshWhole.EV, meshWhole.EF, meshWhole.EFi,
  // meshWhole.innerEdges, VH, EH, FH, HV, HE, HF, nextH, prevH, twinH);
  ...
  for (int i = 0; i < meshWhole.FE.rows(); i++) {
    for (int j = 0; j < 3; j++)
      if (intData.face2cut(i, j)) // face2cut is initalized by
                                  // directional::cut_mesh_with_singularities
  ...
  VectorXi Halfedge2TransitionIndices =
      VectorXi::Constant(meshWhole.dcel.halfedges.size(), 32767);
  VectorXi Halfedge2Matching(meshWhole.dcel.halfedges.size());
  VectorXi isHEClaimed = VectorXi::Zero(meshWhole.dcel.halfedges.size());

  // here we convert the matching that was calculated for the vector field over
  // edges to half-edges
  for (int i = 0; i < meshWhole.dcel.halfedges.size(); i++) {
    // HE is a map between half-edges to edges, but it does not carry the
    // direction EH edge to half-edge mapping
    Halfedge2Matching(i) = (meshWhole.EH(meshWhole.HE(i), 0) == i
                                ? -combedField.matching(meshWhole.HE(i))
                                : combedField.matching(meshWhole.HE(i)));
    if (Halfedge2Matching(i) < 0)
      Halfedge2Matching(i) =
          (intData.N + (Halfedge2Matching(i) % intData.N)) % intData.N;
  }
  ...
  for (int i = 0; i < meshWhole.V.rows(); i++) {
    ...
    if (!isBoundary(i)) {
      // travel throu the start of the vertex and stop once the edge on the cut
      // is found
      do {
        if (isHEcut(currH) != 0)
          break;
    ...
    do {
      ...
      MatrixXi nextPermMatrix =
          constParmMatrices[Halfedge2Matching(nextHalfedge) % intData.N];
      // no update needed
    ...
    // cleaning parmMatrices and permIndices to see if there is a constraint or
    // reveal singularity-from-transition
    std::set<int> cleanPermIndicesSet(permIndices.begin(), permIndices.end());
    std::vector<int> cleanPermIndices(cleanPermIndicesSet.begin(),
                                      cleanPermIndicesSet.end());
      ...
      for (int j = 0; j < cleanPermMatrices.size(); j++) {
        for (int k = 0; k < intData.N; k++)
          for (int l = 0; l < intData.N; l++) {
            constTriplets.emplace_back(intData.N * currConst + k,
                                       intData.N * cleanPermIndices[j] + l,
                                       (double)cleanPermMatrices[j](k, l));
            constTripletsInteger.emplace_back(
                intData.N * currConst + k, intData.N * cleanPermIndices[j] + l,
                cleanPermMatrices[j](k, l));
          }
      }
      currConst++;
  ...
  intData.vertexTrans2CutMat.resize(intData.N * cutV.rows(),
                                    intData.N *
                                        (meshWhole.V.rows() + numTransitions));
  intData.vertexTrans2CutMatInteger.resize(
      intData.N * cutV.rows(),
  ...
  intData.vertexTrans2CutMat.setFromTriplets(cleanTriplets.begin(),
                                             cleanTriplets.end());
  intData.vertexTrans2CutMatInteger.setFromTriplets(
      cleanTripletsInteger.begin(), cleanTripletsInteger.end());

  //

  intData.constraintMat.resize(
      intData.N * currConst, intData.N * (meshWhole.V.rows() + numTransitions));
  intData.constraintMatInteger.resize(
      intData.N * currConst, intData.N * (meshWhole.V.rows() + numTransitions));
  cleanTriplets.clear();
  cleanTripletsInteger.clear();
  ...
  intData.constraintMat.setFromTriplets(cleanTriplets.begin(),
                                        cleanTriplets.end());
  intData.constraintMatInteger.setFromTriplets(cleanTripletsInteger.begin(),
                                               cleanTripletsInteger.end());

  // doing the integer spanning matrix
  intData.intSpanMat.resize(intData.n * (meshWhole.V.rows() + numTransitions),
                            intData.n * (meshWhole.V.rows() + numTransitions));
  intData.intSpanMatInteger.resize(
      intData.n * (meshWhole.V.rows() + numTransitions),
      intData.n * (meshWhole.V.rows() + numTransitions));
  ...
  intData.intSpanMat.setFromTriplets(intSpanMatTriplets.begin(),
                                     intSpanMatTriplets.end());
  intData.intSpanMatInteger.setFromTriplets(intSpanMatTripletsInteger.begin(),
                                            intSpanMatTripletsInteger.end());
  ...
  for (int i = 0; i < intData.N * (meshWhole.V.rows() + numTransitions);
       i += intData.N)
    for (int k = 0; k < intData.N; k++)
      for (int l = 0; l < intData.n; l++) {
        if (intData.linRed(k, l) != 0) {
          linRedMatTriplets.emplace_back(i + k, i * intData.n / intData.N + l,
                                         (double)intData.linRed(k, l));
          linRedMatTripletsInteger.emplace_back(
              i + k, i * intData.n / intData.N + l, intData.linRed(k, l));
        }
      }

  intData.linRedMat.setFromTriplets(linRedMatTriplets.begin(),
                                    linRedMatTriplets.end());
  intData.linRedMatInteger.setFromTriplets(linRedMatTripletsInteger.begin(),
                                           linRedMatTripletsInteger.end());

  // integer variables are per single "d" packet, and the rounding is done for
  // the N functions with projection over linRed
  ...
  if (isSingular.sum() ==
      0) { // no inner singular vertices; vertex 0 is set to (0....0)
    for (int j = 0; j < intData.n; j++)
  ...
  // doing the integer spanning matrix
  intData.singIntSpanMat.resize(
      intData.n * (meshWhole.V.rows() + numTransitions),
      intData.n * (meshWhole.V.rows() + numTransitions));
  intData.singIntSpanMatInteger.resize(
      intData.n * (meshWhole.V.rows() + numTransitions),
      intData.n * (meshWhole.V.rows() + numTransitions));
  ...
  for (int i = 0; i < isSingular.size(); i++) {
    if (!isSingular(i)) {
      for (int j = 0; j < intData.n; j++) {
        singIntSpanMatTriplets.emplace_back(intData.n * i + j,
                                            intData.n * i + j, 1.0);
        singIntSpanMatTripletsInteger.emplace_back(intData.n * i + j,
                                                   intData.n * i + j, 1);
      }
    } else {
      for (int k = 0; k < intData.n; k++)
        for (int l = 0; l < intData.n; l++) {
          if (intData.periodMat(k, l) != 0) {
            singIntSpanMatTriplets.emplace_back(
                intData.n * i + k, intData.n * i + l,
                (double)intData.periodMat(k, l));
            singIntSpanMatTripletsInteger.emplace_back(
                intData.n * i + k, intData.n * i + l, intData.periodMat(k, l));
          }
        }
    }
  }
