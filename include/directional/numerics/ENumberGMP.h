...
public:
  ...
  std::string to_string() const { return value.get_str(); }
  ...
  EInt &operator+=(const EInt &other) {
    value += other.value;
    return *this;
  }
  EInt &operator-=(const EInt &other) {
...
public:
  ...
  ENumber &operator+=(const ENumber &b) {
    value += b.value;
    return *this;
  }
  ENumber &operator/=(const ENumber &b) {
  ...
  long double to_double(int maxDigits = 12) const { return value.get_d(); }
