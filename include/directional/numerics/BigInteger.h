...
public:
  ...
  BigInteger operator+(const BigInteger &other) const {
    ...
    long long carry = 0;
    for (size_t i = 0;
         (i < std::max(digits.size(), other.digits.size())) || carry; i++) {
      long long sum = carry;
      if (i < digits.size())
        sum += digits[i];
  ...
  // Specialized version for multiplying directly with a single digit (effective
  // for division)s
  ...
  // A version that expects the result to be a single digit (like as a part of
  // long division).
  inline long long single_digit_division(const BigInteger &other,
                                         BigInteger &mod) const {
    ...
    if ((this->digits.size() <= CONVERTIBLE_SIZE) &&
        (other.digits.size() <= CONVERTIBLE_SIZE)) {
    ...
    currDividend.digits.resize(1 + dividend.digits.size() -
                               divisor.digits.size());
    ...
    for (int i = 0; i < currDividend.digits.size(); i++)
      currDividend.digits[currDividend.digits.size() - i - 1] =
          dividend.digits[dividend.digits.size() - 1 - i];
